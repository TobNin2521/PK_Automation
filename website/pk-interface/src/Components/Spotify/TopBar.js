...
export const TopBar = ({addTrack, showSettings}) => {
    ...
    useEffect(() => {
        setShowKeyboard(showSearchResults);
    }, [showSearchResults]);
    ...
    return (
        <>
            <div className="top-bar">
                <input className="track-search" type="text" placeholder="Search" value={searchVal} onChange={(e) => setSearchVal(e.target.value)} onFocus={() => setShowSearchResults(true)}/>
            ...
            <KeybordComp showKeyboard={showKeyboard} setKeyboardValue={setSearchVal} onKeyboardPress={keyboardPress} />
        </>
    );
};